...
(function (factory) {
  ...
  var editable = renderer.create('<div className="note-editable card-block" contentEditable="true"/>');
  ...
  var airEditable = renderer.create('<div className="note-editable" contentEditable="true"/>');

  var buttonGroup = renderer.create('<div className="note-btn-group btn-group">');

  var dropdown = renderer.create('<div className="dropdown-menu">', function ($node, options) {
    var markup = $.isArray(options.items) ? options.items.map(function (item) {
      ...
      return '<a className="dropdown-item" href="#" ' + (dataValue + dataOption) + '>' + content + '</a>';
    }).join('') : options.items;
  ...
  var dropdownCheck = renderer.create('<div className="dropdown-menu note-check">', function ($node, options) {
    var markup = $.isArray(options.items) ? options.items.map(function (item) {
      var value = (typeof item === 'string') ? item : (item.value || '');
      var content = options.template ? options.template(item) : item;
      return '<a className="dropdown-item" href="#" data-value="' + value + '">' + icon(options.checkClassName) + ' ' + content + '</a>';
    }).join('') : options.items;
    $node.html(markup);
  });
  ...
  var dialog = renderer.create('<div className="modal" aria-hidden="false" tabindex="-1"/>', function ($node, options) {
    if (options.fade) {
      $node.addClass('fade');
    }
    $node.html([
  ...
  var checkbox = renderer.create('<label className="custom-control custom-checkbox"></label>', function ($node, options) {
      if (options.id) {
          $node.attr('for', options.id);
      }
      $node.html([
          ' <input type="checkbox" className="custom-control-input"' + (options.id ? ' id="' + options.id + '"' : ''),
          (options.checked ? ' checked' : '') + '/>',
          ' <span className="custom-control-indicator"></span>',
          ' <span className="custom-control-description">' + (options.text ? options.text : '') + '</span>',
          '</label>'
      ].join(''));
  });
  ...
  */
  ...
  var Handle = function (context) {
    ...
    this.initialize = function () {
      this.$handle = $([
        ...
        ' note-control-se"></div>',
        (options.disableResizeImage ? '' : '<div className="note-control-selection-info"></div>'),
        '</div>',
        '</div>'
    ...
    this.addToolbarButtons = function () {
      ...
      context.memo('button.fontname', function () {
        return ui.buttonGroup([
          ui.button({
            className: 'dropdown-toggle',
            contents: ui.dropdownButtonContents('<span className="note-current-fontname"/>', options),
      ...
      context.memo('button.fontsize', function () {
        return ui.buttonGroup([
          ui.button({
            className: 'dropdown-toggle',
            contents: ui.dropdownButtonContents('<span className="note-current-fontsize"/>', options),
  ...
  var LinkDialog = function (context) {
    ...
    this.initialize = function () {
      ...
      var body = '<div className="form-group note-form-group">' +
      ...
      (!options.disableLinkTarget ?
          $('<div/>').append(ui.checkbox({ id: 'sn-checkbox-open-in-new-window', text: lang.link.openInNewWindow, checked: true }).render())
              .html()
          : '');
      var footer = '<button href="#" className="btn btn-primary note-btn note-btn-primary ' +
  ...
  var ImageDialog = function (context) {
    ...
    this.initialize = function () {
      ...
      var body = '<div className="form-group note-form-group note-group-select-from-files">' +
  ...
  var VideoDialog = function (context) {
    ...
    this.initialize = function () {
      ...
      var body = '<div className="form-group note-form-group row-fluid">' +
          '<label className="note-form-label">' + lang.video.url + ' <small className="text-muted">' + lang.video.providers + '</small></label>' +
  ...
  var HintPopover = function (context) {
    ...
    this.createGroup = function (idx, keyword) {
      var $group = $('<div className="note-hint-group note-hint-group-' + idx + '"/>');
